...
public class OpenglClass implements GLEventListener, KeyListener, MouseInputListener {
	...
	public void display(GLAutoDrawable gLDrawable) {
		...
		for (int i = 0; i < windowwidth; i++)
			for (int j = 0; j < windowheight; j++) {
				gl.glColor3d(1 - space.position(i, j).getPheromone() / space.maxpheromone.getPheromone(),
						space.position(i, j).getPheromone() / space.maxpheromone.getPheromone(), 0.0);
				gl.glVertex2d(i, j);
	...
	public static void start(double width, double height, Ant[] antarray, SpacePosition[] foodarray, Space space) {
		...
		JFrame frame = new JFrame("Ant optimization - Sam Reynolds, 2019");
